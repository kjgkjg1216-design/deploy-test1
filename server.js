...
const occupations = JSON.parse(fs.readFileSync(path.join(__dirname, 'data', 'occupations.json'), 'utf-8'));
const deaths = JSON.parse(fs.readFileSync(path.join(__dirname, 'data', 'deaths.json'), 'utf-8'));
const eras = JSON.parse(fs.readFileSync(path.join(__dirname, 'data', 'eras.json'), 'utf-8'));
...
app.post('/api/read-past-life', async (req, res) => {
  const { name } = req.body;
  if (!name || !name.trim()) {
...
app.listen(PORT, () => {
  console.log(`전생 리더기 서버 실행 중: http://localhost:${PORT}`);
});
